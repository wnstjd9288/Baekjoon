...
int main() {
    ...
    int n, m;
    long long space_god_cod[1001][2];
    Graph* g = (Graph*)malloc(sizeof(Graph));
    scanf("%d %d", &n, &m);
    init(g, n);

    for (int i = 1; i <= n; i++) {
        scanf("%lld %lld", &space_god_cod[i][0], &space_god_cod[i][1]);
    }
    for (int i = 1; i <= n; i++) {
        for (int j = i + 1; j <= n; j++) {
            double w = sqrt(pow(space_god_cod[i][0] - space_god_cod[j][0], 2) + pow(space_god_cod[i][1] - space_god_cod[j][1], 2));
            graph_push(g, i, j, w);
            graph_push(g, j, i, w);
        }
    }
